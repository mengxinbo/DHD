...
cdef extern from "render/render.h":
  ...
  cdef cppclass BaseRenderer[T]:
    ...
    Buffer[T] buffer;
    BaseRenderer(const Camera[T] cam, const Shader[T] shader, Buffer[T] buffer, int wavelength)
    void render_mesh(const RenderInput[T] input); 
    void render_mesh_proj(const RenderInput[T] input, const Camera[T] proj, const unsigned char* pattern, const float* reflectance, const float* camerasensitivity, const float* illumination, int ref_num, int camsen_num, int illu_num, float d_alpha, float d_beta);

cdef extern from "render/render_cpu.h":
  cdef cppclass RendererCpu[T](BaseRenderer[T]):
    RendererCpu(const Camera[T] cam, const Shader[T] shader, Buffer[T] buffer, int n_threads, int wavelength)
    void render_mesh(const RenderInput[T] input); 
    void render_mesh_proj(const RenderInput[T] input, const Camera[T] proj, const unsigned char* pattern, const float* reflectance, const float* camerasensitivity, const float* illumination, int ref_num, int camsen_num, int illu_num, float d_alpha, float d_beta);
...
cdef class PyRenderInput:
  ...
  def set_colors(self, int[::1] colors):
    self.colors = colors
    cdef int[::1] colors_view = self.colors
    self.input.colors = &colors_view[0]
...
cdef class PyRenderer:
  ...
  def reflectance(self):
   return self.reflectance_buffer

  def normal(self):
   return self.normal_buffer

  def __cinit__(self, PyCamera cam, PyShader shader, int wavelength = 27, engine='cpu', int n_threads=1):
    ...
    self.reflectance_buffer = np.empty((cam.cam[0].height, cam.cam[0].width, wavelength), dtype=np.float32)
    ...
    cdef float[:,:,::1] rbv = self.reflectance_buffer
    cdef float[:,:,::1] nbv = self.normal_buffer
    self.buffer.depth = &dbv[0,0]
    self.buffer.color = &cbv[0,0,0]
    self.buffer.reflectance = &rbv[0,0,0]
    self.buffer.normal = &nbv[0,0,0]

    if engine == 'cpu':
      self.renderer = new RendererCpu[float](cam.cam[0], shader.shader[0], self.buffer, n_threads, wavelength)
    elif engine == 'gpu':
      self.renderer = new RendererGpu[float](cam.cam[0], shader.shader[0], self.buffer, wavelength)
    else:
  ...
  def mesh_proj(self, PyRenderInput input, PyCamera proj, uint8_t[:,:,::1] pattern, float[:,::1] reflectance, float[:,::1] camerasensitivity, float[:,::1] illumination, float d_alpha=1, float d_beta=0):
    if pattern.shape[0] != proj.cam[0].height or pattern.shape[1] != proj.cam[0].width:
      raise Exception(f'pattern has to be a {proj.cam[0].height}x{proj.cam[0].width} tensor')
    self.renderer.render_mesh_proj(input.input, proj.cam[0], &pattern[0,0,0], &reflectance[0,0], &camerasensitivity[0,0], &illumination[0,0], reflectance.shape[0], camerasensitivity.shape[0], illumination.shape[0], d_alpha, d_beta)
