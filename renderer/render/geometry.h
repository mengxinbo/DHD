...
template <typename T>
CPU_GPU_FUNCTION
inline void vec_fill(T* v, const T fill, const int N) {
  for(int idx = 0; idx < N; ++idx) {
    v[idx] = fill;
  }
}
...
template <typename T>
CPU_GPU_FUNCTION
inline void vec_add(const T lam1, const T* in1, const T lam2, const T* in2, T* out, const int N) {
  for(int idx = 0; idx < N; ++idx) {
    out[idx] = lam1 * in1[idx] + lam2 * in2[idx];
  }
}
...
template <typename T, int N=3>
CPU_GPU_FUNCTION
inline T vec_dot_3in(const T* in1, const T* in2, const T* in3) {
  T out = T(0);
  for(int idx = 0; idx < N; ++idx) {
    out += (in1[idx] * in2[idx] * in3[idx]);
  }
  return out;
}
...
template <typename T>
CPU_GPU_FUNCTION
inline T vec_dot_3in(const T* in1, const T* in2, const T* in3, const int N) {
  T out = T(0);
  for(int idx = 0; idx < N; ++idx) {
    out += (in1[idx] * in2[idx] * in3[idx]);
  }
...
template <typename T>
CPU_GPU_FUNCTION
inline T vec_dot(const T* in1, const T* in2, const int N) {
  T out = T(0);
  for(int idx = 0; idx < N; ++idx) {
    out += in1[idx] * in2[idx];
  }
